...
namespace tensorflow {
  ...
  public:
    ...
    env_(env), hdf5_env_(0), hdf5_dset_names_(datasets), row_num_(0), num_rows_(0) {
      ...
      supported_types_ = {H5T_NATIVE_FLOAT, H5T_NATIVE_DOUBLE, H5T_NATIVE_INT, H5T_NATIVE_LONG};
    }
    ...
    Status OnWorkStartedLocked() override {
      ...
      for(unsigned int i=0; i<hdf5_dset_names_.size(); ++i){
        ...
        hid_t dset_id = H5Dopen(hdf5_env_, hdf5_dset_names_[i].c_str(), H5P_DEFAULT);
        HDF5_CHECK_OK(dset_id,"cannot open dataset " + hdf5_dset_names_[i]);
        hdf5_dset_ids_.push_back(dset_id);
                
        //get the memory space id
        hid_t mem_id = H5Dget_space(dset_id);
        HDF5_CHECK_OK(mem_id,"cannot open memory space for dataset "+hdf5_dset_names_[i]);
        hdf5_dset_memids_.push_back(mem_id);
        ...
        H5Sget_simple_extent_dims(mem_id, dims.data(), NULL);
        ...
        hid_t type_id = H5Tget_native_type(H5Dget_type(dset_id),H5T_DIR_DESCEND);
    ...
    Status OnWorkFinishedLocked() override {
      if (hdf5_env_ > 0) {
        ...
        //close everything which is known:
        //types:
        for(unsigned int i=0; i<hdf5_dset_types_.size(); ++i){
          H5Tclose(hdf5_dset_types_[i]);
        }
        //memids:
        for(unsigned int i=0; i<hdf5_dset_memids_.size(); ++i){
          H5Sclose(hdf5_dset_memids_[i]);
        }
        //dataset ids:
        for(unsigned int i=0; i<hdf5_dset_ids_.size(); ++i){
          H5Dclose(hdf5_dset_ids_[i]);
        }
        
        
        //close everything which I have missed
  ...
  private:
    ...
    string ReadRow(const unsigned int& dset_index){
      ...
      hid_t file_space = H5Dget_space(hdf5_dset_ids_[dset_index]);
      ...
      else if(H5Tequal(hdf5_dset_types_[dset_index],H5T_NATIVE_LONG)){
        HDF5_CHECK_OK(H5Dread(hdf5_dset_ids_[dset_index], hdf5_dset_types_[dset_index], mem_space, file_space, plist_id_, reinterpret_cast<long*>(hdf5_dset_buffers_[dset_index])),strings::StrCat("cannot read row ",row_num_," from dataset ",hdf5_dset_names_[dset_index],"."));
      }
